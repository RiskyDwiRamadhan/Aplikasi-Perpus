<?php 
    ...
    WHERE tbtransaksi.status_transaksi = 'Kembali' ORDER BY idtransaksi DESC"; 
    ...
    if(mysqli_num_rows($q_tampil_transaksi) > 0) { 
        ...
        while($r_tampil_transaksi=mysqli_fetch_array($q_tampil_transaksi)) { 
            ...
            $sheet->setCellValue('E'.$index, $r_tampil_transaksi['tglkembali']);
            $sheet->setCellValue('F'.$index, $r_tampil_transaksi['status_transaksi']);

            $index++;
        } // end looping 
    }

    $sheet->setTitle('Data Pengembalian Buku');
